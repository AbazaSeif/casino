...
/**
 * Checks if the user is bust and shows relevant message.
 * @returns {boolean} whether user has busted.
 */
Chips.prototype.bust = function () {
  var bust = this.winnings === 0;
  $(this.game.container).toggleClass('bust', bust);
  if (bust) { this.$status.__('chip.bust'); }
  return bust;
};

/**
 * Called by games to solicit the current bet from the user.
 * @param {jQuery} status html element gives messages to user.
 * @param {function} callback Function to call once a valid bet has been set.
 */
Chips.prototype.start = function (status, callback) {
  status.__('chips.bet-instructions');
  $(this.game.container).addClass('betting');
  this.betsContainer = '#bets-interface--' + this.game.name;
  ...
  $('<div class="stack-group vertical"></div>').appendTo('.spot:not(:has(.stack-group))');

  this.targets = $('.stack-group', this.betsContainer);
...
Chips.prototype.finish = function () {
  ...
  if (total === 0) {
    $('.status', this.betsContainer).__('chips.errors.empty-bet');
  }
  else {
    this.currentBet = total;
    this.status.__('chips.bet-count', total);
    $(this.game.container).removeClass('betting');
...
Chips.prototype.clean = function () {
  ...
  $(this.betsContainer + ',' + this.winningsContainer)
    ...
    .off('dragenter', '.spot, .stack-group', events.chipDragEnter)
    .off('dragleave', '.spot, .stack-group', events.chipDragLeave)
    .off('drop', '.spot, .stack-group', events.chipDragDrop)
    .off('dragover', '.spot, .stack-group', events.chipDragOver)
