...
i18n.addResources('en', 'translation', require('./locale/en.json'));
...
function SampleGame () {
  ...
  this.label = i18n.t('sample-game.name');
...
SampleGame.prototype.restart = function () {
  ...
  $('.status').html(i18n.t(bust ? 'sample-game.bust' : 'sample-game.start'));
};
...
SampleGame.prototype.start = function () {
  ...
  $('.status').html(i18n.t('sample-game.bet-instructions'));
  ...
  this.bets.start( function () {
    ...
    $('.status', self.container)
      .html(i18n.t('sample-game.bet-count', self.bets.currentBet));
...
SampleGame.prototype.end = function (winning) {
  ...
  this.restart();
  $('.status').prepend(i18n.t(winning ? 'sample-game.win' : 'sample-game.lose') + ' ');
};
